...
MATCHER_P3(IsRxStruct, msgTotalSize, buf, bufTotalSize, "") {
  return (arg.msgTotalSize == msgTotalSize)
          && (arg.buf == buf)
          && (arg.bufTotalSize == bufTotalSize);
}
...
TEST(LittleLessProtocolA, rxValidRequestFrame) {
  ...
  EXPECT_CALL(testObj, handleMsgData(llp_MsgType::request, 1, IsRxStruct(1, buf, 1, 0, 1)))
      .Times(1);
  EXPECT_CALL(testObj, handleMsgFinish(llp_MsgType::request, 1, IsRxStruct(1, buf, 1), llp_result::ok))
      .Times(1);
...
TEST(LittleLessProtocolA, rxValidResponseFrame) {
  ...
  EXPECT_CALL(testObj, handleMsgData(llp_MsgType::response, 5, IsRxStruct(2, buf, 2, 0, 2)))
      .Times(1);
  EXPECT_CALL(testObj, handleMsgFinish(llp_MsgType::response, 5, IsRxStruct(2, buf, 2), llp_result::ok))
      .Times(1);
...
TEST(LittleLessProtocolA, rxValidErrorFrame) {
  ...
  EXPECT_CALL(testObj, handleMsgData(llp_MsgType::error, 7, IsRxStruct(3, buf, 5, 0, 3)))
      .Times(1);
  EXPECT_CALL(testObj, handleMsgFinish(llp_MsgType::error, 7, IsRxStruct(3, buf, 5), llp_result::ok))
      .Times(1);
...
TEST(LittleLessProtocolA, rxValidEmptyErrorFrame) {
  ...
  EXPECT_CALL(testObj, handleMsgFinish(llp_MsgType::error, 7, IsMsgTotalSize(0), llp_result::ok))
      .Times(1);
...
TEST(LittleLessProtocolA, rxValidUpdateFrame) {
  ...
  EXPECT_CALL(testObj, handleMsgData(llp_MsgType::update, 254, IsRxStruct(3, buf, 5, 0, 3)))
      .Times(1);
  EXPECT_CALL(testObj, handleMsgFinish(llp_MsgType::update, 254, IsRxStruct(3, buf, 5), llp_result::ok))
      .Times(1);
...
TEST(LittleLessProtocolA, rxValidASCIIFrame) {
  ...
  EXPECT_CALL(testObj, handleMsgData(llp_MsgType::update, 128, IsRxStruct(6, buf, sizeof(buf), 0, 6)))
      .Times(1);
  EXPECT_CALL(testObj, handleMsgFinish(llp_MsgType::update, 128, IsRxStruct(6, buf, sizeof(buf)), llp_result::ok))
      .Times(1);
...
TEST(LittleLessProtocolA, rxValidMixedFrame) {
  ...
  EXPECT_CALL(testObj, handleMsgData(llp_MsgType::update, 128, IsRxStruct(5, buf, sizeof(buf), 0, 5)))
      .Times(1);
  EXPECT_CALL(testObj, handleMsgFinish(llp_MsgType::update, 128, IsRxStruct(5, buf, sizeof(buf)), llp_result::ok))
      .Times(1);
...
TEST(LittleLessProtocolA, rxValidFrameWithEmptyASCII) {
  ...
  EXPECT_CALL(testObj, handleMsgData(llp_MsgType::update, 128, IsRxStruct(3, buf, sizeof(buf), 0, 3)))
      .Times(1);
  EXPECT_CALL(testObj, handleMsgFinish(llp_MsgType::update, 128, IsRxStruct(3, buf, sizeof(buf)), llp_result::ok))
      .Times(1);
...
TEST(LittleLessProtocolA, rxAbortedByApplicationInMiddle) {
  StreamMock sm;
  LittleLessProtocolAMock testObj(sm);
  ...
  EXPECT_CALL(testObj, canHandleMsg(llp_MsgType::update, 128, IsMsgTotalSize(3)))
      .WillOnce(setBuf(buf, sizeof(buf)));
  EXPECT_CALL(testObj, handleMsgData(llp_MsgType::update, 128, IsRxStruct(3, buf, sizeof(buf), 0, 1)))
      .WillOnce(setBuf(NULL, sizeof(buf)));
  EXPECT_CALL(testObj, handleMsgFinish(llp_MsgType::update, 128, IsRxStruct(3, NULL, sizeof(buf)), llp_result::applicationAbort))
      .Times(1);
...
TEST(LittleLessProtocolA, rxAbortedByApplicationAtEnd) {
  StreamMock sm;
  LittleLessProtocolAMock testObj(sm);
...
TEST(LittleLessProtocolA, rxInvalidFrameToLongChecksum) {
  ...
  EXPECT_CALL(testObj, handleMsgData(llp_MsgType::update, 254, IsRxStruct(3, buf, 5, 0, 3)))
      .Times(1);
  EXPECT_CALL(testObj, handleMsgFinish(llp_MsgType::update, 254, IsRxStruct(3, buf, 5),  llp_result::frameError))
      .Times(1);
...
TEST(LittleLessProtocolA, rxInvalidFrameColon3BadChar) {
  ...
  EXPECT_CALL(testObj, canHandleMsg(llp_MsgType::update, 254, IsMsgTotalSize(3)))
      .WillOnce(setBuf(buf, 5));
  EXPECT_CALL(testObj, handleMsgFinish(llp_MsgType::update, 254, _, llp_result::frameError))
      .Times(1);
